...
def find_nearest_lane(prediction, center, selected_y):
    ...
    blue_pixels = np.where((prediction[selected_y] == [0, 0, 255]).all(axis=-1))
...
def show_seg_result(img, result, palette=None,is_demo=True):
    ...
    # Define ROI based on the extracted lane lines
    lane_lines = np.where(complete_lane_lines == 1)
    min_y = np.min(lane_lines[0]) # Set the top boundary at half of the image height
    top_left = (width // 2 - 50, min_y)
    top_right = (width // 2 + 50, min_y)
    ...
    bir_eye_result = warp_perspective(color_area, src_points, dst_points, (width, height))

    for i in range(0, height, 20):
        # Find nearest left and right lane x-coordinates for the current row
        left_nearest_x, right_nearest_x = find_nearest_lane(bir_eye_result, center, i)
    ...
    try:
        ...
        cv2.polylines(output_img, [np.array(list(zip(left_fit_x, plot_y)), np.int32)], False, (255, 0, 0), thickness=5)
        cv2.putText(output_img, 'Left Lane Curvature: {:.2f} m'.format(left_curve), (50, 100), font, 1, (255, 255, 255), 2, cv2.LINE_AA)
    except:
        print()
    try:
        # Calculate the intercept points at the bottom of our image
        ...
        # Draw the detected lane lines on the output image
        cv2.polylines(output_img, [np.array(list(zip(right_fit_x, plot_y)), np.int32)], False, (255, 0, 0), thickness=5)
        cv2.putText(output_img, 'Right Lane Curvature: {:.2f} m'.format(right_curve), (50, 150), font, 1, (255, 255, 255), 2, cv2.LINE_AA)
    except:
        print("hey")
