...
class TuSimple(object):
    ...
    def get_metrics(self, lanes, idx):
        ...
        _, _, _, matches, accs, dist = LaneEval.bench(
    ...
    def load_annotations(self):
        ...
        for anno_file in self.anno_files:
            ...
            for line in lines:
                ...
                self.max_points = max(
                    self.max_points, max([len(l) for l in gt_lanes]))
    ...
    def save_tusimple_predictions(self, predictions, runtimes, filename):
        ...
        for idx in range(len(predictions)):
            line = self.pred2tusimpleformat(
                idx, predictions[idx], runtimes[idx])
            lines.append(line)
    ...
    def eval(self, exp_dir, predictions, runtimes, label=None, only_metrics=False):
        ...
        if self.metric == 'default':
            result = json.loads(LaneEval.bench_one_submit(
                pred_filename, self.anno_files[0]))
        elif self.metric == 'ours':
            result = json.loads(
                eval_json(pred_filename, self.anno_files[0], json_type='tusimple'))
        table = {}
